...
def users():
    ...
    mysql = connectToMySQL("registration")
    query = "SELECT following FROM followers WHERE follower = %(fi)s"
    data = {
        'fi' : session['user_id']
    }
    ...
    mysql = connectToMySQL('registration')
    query = "SELECT users.first_name, users.last_name, users.email, users.id_user FROM followers JOIN users ON followers.follower = %(id)s"
    data = {
        'id': session['user_id']
    }
    following_list = mysql.query_db(query)

    return render_template('users.html', user_data=user_data, all_users=all_users, follow_list=follow_list)

@app.route("/follow/<id_user>")
def follow(id_user):
    mysql = connectToMySQL('registration')
    query = "INSERT INTO followers (follower, following) VALUES (%(follower)s, %(following)s)"
    data = {
...
@app.route("/unfollow/<id_user>")
def unfollow(id_user):
    mysql = connectToMySQL('registration')
    query = "DELETE FROM followers WHERE following = %(fol)s"
    data = {
        'fol': id_user
    }
    result = mysql.query_db(query, data)
...
@app.route("/followers/<id_user>")
def followers(id_user):
    mysql = connectToMySQL('registration')
    query = "SELECT * FROM users WHERE id_user = %(id)s"
    data = {
        'id' : session['user_id']
    ...
    mysql = connectToMySQL('registration')
    query = "SELECT followers.follower, followers.following, users.first_name, users.last_name, users.email FROM followers JOIN users ON followers.following = users.id_user"
    following_user = mysql.query_db(query)
    print(following_user)
    

    return render_template("follow.html", user_data=user_data, following_user=following_user)
...
def success():
    ...
    feed_tweets = []
    id_following = mysql.query_db(query, data)
    for tweet in all_tweets:
        if tweet['author'] in [following['following'] for following in id_following]:
    ...
    return render_template("login.html", user_data=user_data, all_tweets=feed_tweets, now=now, liked_tweets=liked_tweets, like_count=like_count)
